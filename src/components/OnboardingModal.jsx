...
export default function OnboardingModal({ isOpen, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    await onSubmit({ shopName, ownerName });
    setIsLoading(false);
  };
      ...
      <div className="relative bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-fade-in p-8">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-slate-500 text-sm">
            Sebelum mulai menghitung, beri tahu kami sedikit tentang bisnis Anda.
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700">Nama Panggilan / Pemilik</label>
            <input 
              type="text" 
              required
              autoFocus
              placeholder="Contoh: Bayu, Andi..."
